      ...
      {tabs.map(({ to, label, icon: Icon, badge }) => (
        ...
        >
          <Icon
            size={17}
            strokeWidth={2.25}
            className="transition-colors text-ink-400 group-hover:text-ink-700 group-[.tab-active]:text-brand-500"
          />
          <span>{label}</span>
          {badge && (
          ...
          <span
            className="absolute left-3 right-3 -bottom-px h-[2px] rounded-full transition-all duration-300 bg-transparent group-hover:bg-ink-200 group-[.tab-active]:bg-brand-500"
            aria-hidden
          />
        </NavLink>
      ))}
